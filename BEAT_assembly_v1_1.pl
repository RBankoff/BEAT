...
sub COMPLETE{
	...
	foreach $O(@odds){
		my $SRQ;
		$SRQ = $O;
		$SRQ .= "." . (split(/\./, $ref))[0] . ".map_script";
		print "SRQ: $SRQ\n";
		my $local_bam_base = (split(/\./, $O))[0];
		$local_bam_base .= "." . (split(/\./, $ref))[0];
		...
		if ($type =~ m/qsub/){
			my $job_catcher = qx(qsub $SRQ);
			chomp $job_catcher;
			#print "Job catcher: $job_catcher\n";
        		my $actual_job_no = (split(/\./, $job_catcher))[0];
        		$Job_numbers{$O} = $actual_job_no;
		}
		$SR_counter++;
	}
	if ($type =~ m/qsub/i){
		foreach $O(@odds){
			my $local_job = $Job_numbers{$O};
		    	my $qstatchecker = qx(qstat -u $usr);
		        if ($qstatchecker =~ m/$local_job/g){
				$qstatchecker = qx(qstat -u $usr);
				until($qstatchecker !~ m/$local_job/g){
	                		$qstatchecker = qx(qstat -u $usr);
	                	       	sleep(2);
				}
	                }
	...
	unless ($numSR == 2){
		...
		if ($type =~ m/qsub/){
			my $job_catcher = qx(qsub $merging_script);
                        chomp $job_catcher;
                        #print "Job catcher: $job_catcher\n";
                        my $actual_job_no = (split(/\./, $job_catcher))[0];
                        $Job_numbers{$merged_name} = $actual_job_no;

		        my $local_job = $Job_numbers{$merged_name};
                        my $qstatchecker = qx(qstat -u $usr);
                        if ($qstatchecker =~ m/$local_job/g){
                                $qstatchecker = qx(qstat -u $usr);
                                until($qstatchecker !~ m/$local_job/g){
	...
	my $rmdup_index_script = $job_name . ".rmdup_script";
	my $rmdup_name = $job_name . ".rmdup.bam";
	open (RMDUPING_SCRIPT, ">", $rmdup_index_script) or die "Couldn't make rmdup file $rmdup_index_script, $!\n";
	print RMDUPING_SCRIPT "$config_header\n";
	print RMDUPING_SCRIPT "module load samtools\n";
	print RMDUPING_SCRIPT "samtools rmdup -S $merged_prermdup $rmdup_name\n";
	print RMDUPING_SCRIPT "samtools index $rmdup_name\n";
	push @discard, $rmdup_index_script;
	
	if ($type =~ m/qsub/){
		my $job_catcher = qx(qsub $rmdup_index_script);
                chomp $job_catcher;
                #print "Job catcher: $job_catcher\n";
                my $actual_job_no = (split(/\./, $job_catcher))[0];
                $Job_numbers{$rmdup_name} = $actual_job_no;

                my $local_job = $Job_numbers{$rmdup_name};
                my $qstatchecker = qx(qstat -u $usr);
                if ($qstatchecker =~ m/$local_job/g){
                	$qstatchecker = qx(qstat -u $usr);
                        until($qstatchecker !~ m/$local_job/g){
...
sub FAST_MAP{
	...
	foreach $l(@localnames){
		...
		print BWA "samtools rmdup -S $local_bam_sorted $local_bam_rmdupd \n";
		print BWA "samtools index $local_bam_rmdupd \n";
		close BWA;
